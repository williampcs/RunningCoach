...
### 4.2 laps_json 與 streams_json 說明
...
**注意**：laps 與 streams 各需一次額外 API call，會在 `/sync` 時一次拉取，跑後回報流程的即時拉取（`fetch_latest_activity`）只取基本活動資料，不拉 laps/streams，避免延遲回應。laps/streams 在後續 `/sync` 時補入。

### 4.3 workout_summaries 摘要格式
...
AI 評估：第7組後配速下滑且心率上升幅度偏大，當前有氧底子可能限制恢復速度，
...
- [ ] `db.py`：SQLite 初始化，建立所有 table（含 v1.2 所有欄位）
