...
層1包含三個部分：動態時間資訊、固定選手資料（來自 `athlete_profile`）、動態注入的賽事資訊（來自 `races`）。每次呼叫 API 前即時組合。

**動態時間資訊**（每次呼叫時即時生成，時區 Asia/Taipei）：
```
今天日期：2026-04-03（星期五）｜時區：Asia/Taipei（UTC+8）｜以星期日為一週的第一天
```

注入原因：LLM 本身不知道當前日期，若不注入會以訓練資料的時間印象回答，導致訓練計畫建議錯誤。星期日為一週第一天符合跑步訓練計畫的常見慣例（週日長跑 → 週一休息）。

長期目標欄位（`goal_long_term`）為自由格式字串，可包含多個目標，例如：「半馬：2026年底破二；全馬：2027年底破430」。
